...
function ClubSchedule() {
    const [clickedTab, setClickedTab] = useState<string|null>("")

    const tabHandler = (e:React.MouseEvent<HTMLElement>) => {
        setClickedTab(e.currentTarget.getAttribute("name"));
        e.currentTarget.setAttribute("className", "clicked")
    }
    return (
        <S_Container>
            <div>
                <S_Button>새 경기 또는 지난 경기 등록하기 +</S_Button>
            </div>
            <S_TapMenu>
                <S_TabButton name="scheduled" onClick={tabHandler}>예정 경기</S_TabButton>
                <S_TabButton name="past" onClick={tabHandler}>지난 경기</S_TabButton>
            </S_TapMenu>
            <S_MatchSchedule>
